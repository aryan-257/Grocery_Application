...
public class AppDbContext(DbContextOptions<AppDbContext> options) : DbContext(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder mb)
    {
        ...
        mb.Entity<Payment>().Property(p => p.Amount).HasColumnType("decimal(18,2)");
        mb.Entity<Payment>().HasIndex(p => p.RazorpayOrderId).IsUnique();
        mb.Entity<Payment>().HasOne(p => p.User).WithMany().HasForeignKey(p => p.UserId);
        mb.Entity<Payment>().HasOne(p => p.Order).WithMany().HasForeignKey(p => p.OrderId);
    }
}
