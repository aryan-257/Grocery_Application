      ...
      <div class="max-w-5xl mx-auto px-4 py-8">
        ...
        @if (loading()) {
          ...
          <div class="mt-10 bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl p-6">
            ...
            <!-- Write review form -->
            @if (canReview()) {
              <div class="border-2 border-green-300 dark:border-green-700 rounded-2xl p-5 mb-6 bg-green-50 dark:bg-green-900/10">
                <p class="text-sm font-bold text-gray-800 dark:text-gray-100 mb-4">Write a Review</p>
                <!-- Star selector -->
                <div class="flex gap-2 mb-4">
                  ...
                  <span class="ml-2 text-sm font-semibold text-gray-600 dark:text-gray-300 self-center">
                    {{ reviewRating() }}/5 — {{ ratingLabel() }}
                  </span>
                </div>
                <textarea [(ngModel)]="reviewComment" rows="3" placeholder="Share your experience with this product..."
                  class="w-full bg-white dark:bg-gray-800 border-2 border-gray-300 dark:border-gray-600 rounded-xl px-4 py-3 text-sm text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:border-green-500 resize-none transition mb-3"></textarea>
                @if (reviewError()) {
                  <p class="text-xs text-red-500 mb-2">{{ reviewError() }}</p>
                }
                <button (click)="submitReview()" [disabled]="reviewSubmitting()"
                  class="bg-green-600 hover:bg-green-700 disabled:opacity-50 text-white text-sm font-bold px-6 py-2.5 rounded-xl transition shadow-sm hover:shadow-md">
                  {{ reviewSubmitting() ? 'Submitting...' : 'Submit Review' }}
                </button>
              </div>
            } @else if (alreadyReviewed()) {
              <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 text-blue-700 dark:text-blue-400 rounded-xl px-4 py-3 text-sm mb-5 font-medium">
                &#x2705; You have already reviewed this product.
              </div>
            } @else if (!auth.isAuthenticated()) {
              <div class="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 text-amber-700 dark:text-amber-400 rounded-xl px-4 py-3 text-sm mb-5 font-medium">
                Please <a routerLink="/auth/login" class="underline font-bold">login</a> to write a review.
              </div>
            } @else {
              <div class="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 rounded-xl px-4 py-3 text-sm mb-5">
                You need to order this product before reviewing it.
              </div>
            }
                ...
                @for (p of related(); track p.id) {
                  <div (click)="goTo(p.id)"
                    ...
                    <div class="h-36 overflow-hidden">
                      <img [src]="p.imageUrl" [alt]="p.name" class="w-full h-full object-contain group-hover:scale-105 transition-transform duration-300" />
                    </div>
                    <div class="p-3">
